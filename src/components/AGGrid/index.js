...
import { exportToExcelFull } from '../../utils/excel';
import AGGridGroup from './group';
    ...
    if (validations !== undefined && mostrarpaleta && params.data) {
      ...
      if (!isSelected) {
        if (valida) {
          ...
          if (mostrarpaleta) {
            return params.node.rowIndex % 2 !== 0 ? { backgroundColor: 'rgb(206, 235, 253)' } : {};
          }
        }
      } else {
        return { backgroundColor: 'rgb(4, 168, 244)', color: '#fff' };
      }
    } else {
      if (!isSelected) {
        if (mostrarpaleta) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Box display="flex" marginTop={'5px'} justifyContent="flex-end" gap={1} mb={1}>
            ...
            {!multselec ? (
              <Tooltip title={checked ? 'Multi-seleção ativada' : 'Multi-seleção desativada'}>
                <IconButton onClick={() => setChecked(!checked)}>{checked ? <CheckBoxOutlineBlank /> : <CheckBox />}</IconButton>
              </Tooltip>
            ) : (
              <></>
            )}
            ...
            {permgroup && isnumber ? (
              <Tooltip title="Agrupamento" arrow>
                <IconButton size="small" onClick={() => setShowgroup(true)}>
                  <FolderSpecialIcon />
                </IconButton>
              </Tooltip>
            ) : (
          ...
          <span style={{ fontSize: '13px', color: '#333' }}>Total de linhas: {!checked && !filtered ? rows.length : rowsselec.length}</span>
        </div>
      </div>
      <Modal backdrop="static" size="lg" show={showcolumns} centered={true} onHide={handleClosecolumns}>
        ...
        <Modal.Footer>
          <Button id="btnFechar" className="btn btn-primary shadow-2 mb-3" onClick={(e) => handleClosecolumns()}>
      ...
      <Modal backdrop="static" show={showtotal} onHide={() => setShowtotal(false)}>
            ...
            <div key={index} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
              ...
              <h5
                style={{
                  flex: 1,
                  textAlign: 'right',
                  color: '#007bff', // Cor destacada para o valor
                  fontWeight: 'bold'
                }}
              >
        ...
        <Modal.Footer>
          <Button id="btnFechar" className="btn btn-primary shadow-2 mb-3" onClick={(e) => setShowtotal(false)}>
            <i className={'feather icon-log-out'} /> Fechar
          </Button>
        </Modal.Footer>
      </Modal>
      ...
      <Modal backdrop="static" size="xl" show={showgroup} centered={true} onHide={handleClosegroup}>
        <Modal.Header className="h5" closeButton>
        ...
        <ModalBody>
          <AGGridGroup
            ...
            columns={columnDefs}
            showgroup={showgroup}
            setShowgroup={(data) => setShowgroup(false)}
            modulo={modulo}
          ></AGGridGroup>
        </ModalBody>
      </Modal>
