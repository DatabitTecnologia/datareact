...
const CustoItem = (props) => {
  const { cabecalho, itemselec, classitem } = props;
  const { showcusto, setShowcusto } = props;
  ...
  useEffect(() => {
    setValuesfield([itemselec.custo]);
  }, [itemselec]);
  ...
  const Salvar = () => {
    ...
    itemsave['custo'] = parseFloat(valuesfield[0]);
  ...
  return (
    <React.Fragment>
      <div id="frmcusto" name="frmcusto">
        <div id="linear-progress">{carregando && <LinearProgress color="primary" />}</div>
        <Card className="Recent-Users" style={{ marginBottom: '2px' }}>
          <Card.Header>
            <Card.Title as="h5">Custo do Item</Card.Title>
          </Card.Header>
          <Card.Body>
            <Row>
