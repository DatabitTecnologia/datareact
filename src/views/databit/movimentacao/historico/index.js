...
const MovimentacaoHistorico = (props) => {
  ...
  useEffect(() => {
    ...
    setColumns([
      ...
      { headerClassName: 'header-list', field: 'user', headerName: 'Usuário', width: 235 },
      { headerClassName: 'header-list', field: 'status', headerName: 'Status', width: 60 },
      { headerClassName: 'header-list', field: 'nome', headerName: 'Descrição Status', width: 620 }
    ]);
    setCarregando(true);
    apiList(
      'Historico',
      '*',
      '',
      "TB02130_CODIGO = '" + props.codigo + "' AND TB02130_TIPO = '" + props.tipo + "' order by TB02130_DATA desc "
    ).then((response) => {
