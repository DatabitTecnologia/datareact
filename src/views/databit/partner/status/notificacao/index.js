...
const PartnerNotificacao = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList(
      ...
      '*',
      '',
      "TB00035_SITUACAO = 'A' AND NOT EXISTS (SELECT TB01151_USER FROM TB01151 WHERE TB01151_USER = TB00035_NOME AND TB01151_STATUS =  '" +
        props.statusselec +
    ...
    apiList('PartnerNotiication', '*', 'TB01151_USER AS nome', "TB01151_STATUS= '" + props.statusselec + "' order by TB01151_USER ").then(
      (response) => {
        if (response.status === 200) {
          //console.log(response.data);
  ...
  const Salvar = () => {
    ...
    apiExec("DELETE FROM TB01151 WHERE TB01151_STATUS = '" + props.statusselec + "' ", 'N').then((response) => {
      if (response.status === 200) {
        rowsselect.forEach((item) => {
          iteminsert['user'] = item.nome;
          iteminsert['status'] = props.statusselec;
          apiInsert('PartnerNotiication', iteminsert).then((response) => {
            if (response.status === 200) {
              setStatusprocessa('');
            }
          });
...
export default PartnerNotificacao;
