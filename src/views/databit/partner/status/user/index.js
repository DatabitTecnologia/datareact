...
const PartnerUser = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList(
      ...
      '*',
      '',
      "TB00035_SITUACAO = 'A' AND NOT EXISTS (SELECT TB01150_USER FROM TB01150 WHERE TB01150_USER = TB00035_NOME AND TB01150_STATUS =  '" +
        props.statusselec +
    ...
    apiList('PartnerUser', '*', 'TB01150_USER AS nome', "TB01150_STATUS= '" + props.statusselec + "' order by TB01150_USER ").then(
      (response) => {
        if (response.status === 200) {
          //console.log(response.data);
  ...
  const Salvar = () => {
    ...
    apiExec("DELETE FROM TB01150 WHERE TB01150_STATUS = '" + props.statusselec + "' ", 'N').then((response) => {
      if (response.status === 200) {
        rowsselect.forEach((item) => {
          iteminsert['user'] = item.nome;
          iteminsert['status'] = props.statusselec;
          apiInsert('PartnerUser', iteminsert).then((response) => {
            if (response.status === 200) {
              setStatusprocessa('');
            }
          });
