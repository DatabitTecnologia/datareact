...
const PartnerWorkflow = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList(
      'PartnerStatus',
      'TB01148_CODIGO,TB01148_NOME',
      '',
      "TB01148_SITUACAO = 'A' AND TB01148_CODIGO <> '" +
        props.statusselec +
        "' " +
        "AND NOT EXISTS (SELECT TB01149_STATUS FROM TB01149 WHERE TB01149_STATUSFIM = TB01148_CODIGO AND TB01149_STATUS =  '" +
        props.statusselec +
        "') order by TB01148_NOME"
    ).then((response) => {
    ...
    apiList('PartnerWorkflowVW', '*', '', "TB01149_STATUS= '" + props.statusselec + "' order by TB01148_NOME ").then((response) => {
      if (response.status === 200) {
  ...
  const Salvar = () => {
    ...
    apiExec("DELETE FROM TB01149 WHERE TB01149_STATUS = '" + props.statusselec + "' ", 'N').then((response) => {
      if (response.status === 200) {
        rowsselect.forEach((item) => {
          iteminsert['statusfim'] = item.codigo;
          iteminsert['status'] = props.statusselec;
          apiInsert('PartnerWorkflow', iteminsert).then((response) => {
            if (response.status === 200) {
              setStatusprocessa('');
            }
          });
