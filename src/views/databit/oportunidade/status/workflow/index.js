...
const OportunidadeWorkflow = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList(
      'OportunidadeStatus',
      'TB01129_CODIGO,TB01129_NOME',
      '',
      "TB01129_SITUACAO = 'A' AND TB01129_CODIGO <> '" +
        props.statusselec +
        "' " +
        "AND NOT EXISTS (SELECT TB01130_STATUS FROM TB01130 WHERE TB01130_STATUSFIM = TB01129_CODIGO AND TB01130_STATUS =  '" +
        props.statusselec +
        "') order by TB01129_NOME"
    ).then((response) => {
    ...
    apiList('OportunidadeWorkflowVW', '*', '', "TB01130_STATUS= '" + props.statusselec + "' order by TB01129_NOME ").then((response) => {
      if (response.status === 200) {
  ...
  const Salvar = () => {
    ...
    apiExec("DELETE FROM TB01130 WHERE TB01130_STATUS = '" + props.statusselec + "' ", 'N').then((response) => {
      if (response.status === 200) {
        rowsselect.forEach((item) => {
          iteminsert['statusfim'] = item.codigo;
          iteminsert['status'] = props.statusselec;
          apiInsert('OportunidadeWorkflow', iteminsert).then((response) => {
            if (response.status === 200) {
              setStatusprocessa('');
            }
          });
...
export default OportunidadeWorkflow;
