...
const OportunidadeUser = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList(
      ...
      '*',
      '',
      "TB00035_SITUACAO = 'A' AND NOT EXISTS (SELECT TB01131_USER FROM TB01131 WHERE TB01131_USER = TB00035_NOME AND TB01131_STATUS =  '" +
        props.statusselec +
    ...
    apiList('OportunidadeUser', '*', 'TB01131_USER AS nome', "TB01131_STATUS= '" + props.statusselec + "' order by TB01131_USER ").then(
      (response) => {
        if (response.status === 200) {
          //console.log(response.data);
  ...
  const Salvar = () => {
    ...
    apiExec("DELETE FROM TB01131 WHERE TB01131_STATUS = '" + props.statusselec + "' ", 'N').then((response) => {
      if (response.status === 200) {
        rowsselect.forEach((item) => {
          iteminsert['user'] = item.nome;
          iteminsert['status'] = props.statusselec;
          apiInsert('OportunidadeUser', iteminsert).then((response) => {
            if (response.status === 200) {
              setStatusprocessa('');
            }
          });
...
export default OportunidadeUser;
