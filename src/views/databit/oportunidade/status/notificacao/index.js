...
const OportunidadeNotificacao = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList(
      ...
      '*',
      '',
      "TB00035_SITUACAO = 'A' AND NOT EXISTS (SELECT TB01143_USER FROM TB01143 WHERE TB01143_USER = TB00035_NOME AND TB01143_STATUS =  '" +
        props.statusselec +
    ...
    apiList(
      ...
      '*',
      'TB01143_USER AS nome',
      "TB01143_STATUS= '" + props.statusselec + "' order by TB01143_USER "
    ).then((response) => {
      if (response.status === 200) {
        //console.log(response.data);
  ...
  const Salvar = () => {
    ...
    apiExec("DELETE FROM TB01143 WHERE TB01143_STATUS = '" + props.statusselec + "' ", 'N').then((response) => {
      if (response.status === 200) {
        rowsselect.forEach((item) => {
          iteminsert['user'] = item.nome;
          iteminsert['status'] = props.statusselec;
          apiInsert('OportunidadeNotificacao', iteminsert).then((response) => {
            if (response.status === 200) {
              setStatusprocessa('');
            }
          });
...
export default OportunidadeNotificacao;
