...
const PropostaEndereco = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList(
      ...
      setColumns([
        ...
        { headerClassName: 'header-list', field: 'nome', headerName: 'Descrição do Endereço', width: 384 },
  ...
  const Incluir = () => {
    apiID('PropostaEndereco').then((response) => {
      if (response.status === 200) {
    ...
    if (rows.length > 0 && valuesfield[0] !== '' && valuesfield[0] !== undefined) {
      Confirmation('frmendereco', 'Confirma a exclusão deste registro ?').then((result) => {
        if (result.isConfirmed) {
          setCarregando(true);
          apiDelete('PropostaEndereco', itemselec).then((response) => {
            if (response.status === 200) {
              setCarregando(false);
  ...
  const Salvar = () => {
    if (document.getElementById('TB02263_NOME').value === undefined || document.getElementById('TB02263_NOME').value === '') {
      setItemvariant(1);
      ...
      try {
        ...
        if (inclusao) {
          ...
          apiUpdate('PropostaEndereco', item).then((response) => {
            if (response.status === 200) {
              setCarregando(false);
              if (response.data.status === 1) {
                setDisabled(response.data.status === 1);
...
export default PropostaEndereco;
