...
const OportunidadeTipoStatus = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList(
      ...
      '',
      "TB01129_SITUACAO = 'A' AND NOT EXISTS (SELECT TB01154_STATUS FROM TB01154 WHERE TB01154_STATUS = TB01129_CODIGO AND TB01154_TIPO =  '" +
        props.tipo +
    ...
    apiList('OportunidadeTipoStatusVW', '*', 'TB01154_STATUS as codigo', "TB01154_TIPO= '" + props.tipo + "' order by TB01129_NOME ").then(
      (response) => {
        if (response.status === 200) {
  ...
  const Salvar = () => {
    ...
    apiExec("DELETE FROM TB01154 WHERE TB01154_TIPO = '" + props.tipo + "' ", 'N').then((response) => {
      if (response.status === 200) {
        rowsselect.forEach((item) => {
          iteminsert['status'] = item.codigo;
          iteminsert['tipo'] = props.tipo;
          apiInsert('OportunidadeTipoStatus', iteminsert).then((response) => {
            if (response.status === 200) {
              setStatusprocessa('');
            }
          });
...
export default OportunidadeTipoStatus;
