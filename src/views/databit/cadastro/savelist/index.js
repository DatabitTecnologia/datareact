...
const SaveList = (props) => {
  ...
  useEffect(() => {
    setColumns([
      { headerClassName: 'header-list', field: 'ordem', headerName: 'Ordem', width: 60 },
      { headerClassName: 'header-list', field: 'campo', headerName: 'Nome Campo', width: 150 },
      { headerClassName: 'header-list', field: 'funcao', headerName: 'Descrição do Campo', width: 537 }
    ]);
  }, []);
  ...
  const SalvarOpcao = () => {
    ...
    if (parseInt(valuesfield[0]) === 0) {
      if (valuesfield[2] !== '' && valuesfield[2] !== undefined) {
        ...
        apiInsert('Optionlist', item).then((response) => {
          if (response.status === 200) {
            let idoption = response.data.id;
            let itemadd = {};
            apiExec("DELETE FROM TB00104 WHERE TB00104_OPTION = '" + idoption + "' ", 'N').then((response) => {
              if (response.status === 200) {
                rowsselect.forEach((item) => {
                  itemadd = { option: idoption, campo: item.campo, ordem: item.ordem, selec: 'S', funcao: item.funcao };
      ...
      let itemadd = {};
      apiExec("DELETE FROM TB00104 WHERE TB00104_OPTION = '" + valuesfield[3] + "' ", 'N').then((response) => {
        if (response.status === 200) {
          rowsselect.forEach((item) => {
            itemadd = { option: valuesfield[3], campo: item.campo, ordem: item.ordem, selec: 'S', funcao: item.funcao };
            apiInsert('OptionlistField', itemadd).then((response) => {
              if (response.status === 200) {
                setCarregando(false);
                setShowsave(false);
