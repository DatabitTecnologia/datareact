...
const FieldsRequired = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList(
      ...
      'TB00003_CAMPO, TB00003_FUNCAO, TB00003_SELEC3, TB00003_TABELA',
      '',
      "TB00003_TABELA = '" + props.object + "' and TB00003_SELEC3 = 'S' ORDER BY TB00003_FUNCAO"
    ).then((response) => {
      if (response.status === 200) {
  ...
  useEffect(() => {
    props.setOptiontitle('Obrigatóriedade');
    setColumns([
  ...
  const Salvar = () => {
    ...
    apiExec("UPDATE TB00003 SET TB00003_SELEC3 = 'N' WHERE TB00003_TABELA = '" + props.object + "' ", 'N').then((response) => {
      if (response.status === 200) {
          ...
          <div style={{ width: '4%', marginLeft: '20px' }}>
            <Row>
              {itemselect !== undefined ? (
                <Button id="btnAddobr" className="btn-icon" onClick={() => Add()}>
              ...
              {itemselect2 !== undefined ? (
                <Button id="btnRetobr" className="btn-icon" onClick={() => Subtract()}>
