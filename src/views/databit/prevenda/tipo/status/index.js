...
const PrevendaTipoStatus = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList(
      ...
      '',
      "TB01156_SITUACAO = 'A' AND NOT EXISTS (SELECT TB01161_STATUS FROM TB01161 WHERE TB01161_STATUS = TB01156_CODIGO AND TB01161_TIPO =  '" +
        props.tipo +
    ...
    apiList('PrevendaTipoStatusVW', '*', 'TB01161_STATUS as codigo', "TB01161_TIPO= '" + props.tipo + "' order by TB01156_NOME ").then(
      (response) => {
        if (response.status === 200) {
  ...
  const Salvar = () => {
    ...
    apiExec("DELETE FROM TB01161 WHERE TB01161_TIPO = '" + props.tipo + "' ", 'N').then((response) => {
      if (response.status === 200) {
        rowsselect.forEach((item) => {
          iteminsert['status'] = item.codigo;
          iteminsert['tipo'] = props.tipo;
          apiInsert('PrevendaTipoStatus', iteminsert).then((response) => {
            if (response.status === 200) {
              setStatusprocessa('');
            }
          });
...
export default PrevendaTipoStatus;
