...
const PrevendaUser = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList(
      ...
      '*',
      '',
      "TB00035_SITUACAO = 'A' AND NOT EXISTS (SELECT TB01158_USER FROM TB01158 WHERE TB01158_USER = TB00035_NOME AND TB01158_STATUS =  '" +
        props.statusselec +
    ...
    apiList('PrevendaUser', '*', 'TB01158_USER AS nome', "TB01158_STATUS= '" + props.statusselec + "' order by TB01158_USER ").then(
      (response) => {
        if (response.status === 200) {
          //console.log(response.data);
  ...
  const Salvar = () => {
    ...
    apiExec("DELETE FROM TB01158 WHERE TB01158_STATUS = '" + props.statusselec + "' ", 'N').then((response) => {
      if (response.status === 200) {
        rowsselect.forEach((item) => {
          iteminsert['user'] = item.nome;
          iteminsert['status'] = props.statusselec;
          apiInsert('PrevendaUser', iteminsert).then((response) => {
            if (response.status === 200) {
              setStatusprocessa('');
            }
          });
