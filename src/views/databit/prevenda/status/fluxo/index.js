...
import StatusInvite from '../../../oportunidade/status/invite';
...
const StatusFluxo = (props) => {
  ...
  useEffect(() => {
    ...
    apiFind('Prevenda', '*', '', "TB02303_CODIGO = '" + props.prevenda + "' ").then((response) => {
  ...
  useEffect(() => {
    if (itemselec !== undefined && itemselec !== null) {
      ...
      let filterstatus =
        " AND (TB01156_CODIGO IN ( SELECT TB01157_STATUSFIM FROM TB01157 WHERE TB01157_STATUS = '" + valuesfield[0] + "') ";
      ...
      filterstatus = filterstatus + " ) or (TB01156_CODIGO = '" + valuesfield[0] + "')";
  ...
  useEffect(() => {
    ...
    apiFind('PrevendaStatus', '*', '', "TB01156_CODIGO = '" + status + "' ").then((response) => {
      if (response.status === 200) {
        setCarregando(false);
  ...
  const Salvar = () => {
    if (document.getElementById('TB02303_STATUS2').value === undefined || document.getElementById('TB02303_STATUS2').value === '') {
      setItemvariant(1);
      setMensagem('Campo Novo Status é preenchimento obrigatório !');
      document.getElementById('TB02303_STATUS2').focus();
    } else {
      try {
        setCarregando(true);
        let libera = true;
        if (propstatus.valor1 + propstatus.valor2 > 0) {
          if (!(itemselec.vlrnota >= propstatus.valor1 && itemselec.vlrnota <= propstatus.valor2)) {
            libera = false;
            setCarregando(false);
            setItemvariant(1);
        ...
        if (libera) {
          Password(
            ...
            if (result.isConfirmed) {
              gerarOrcamento(itemselec.codigo, propstatus, 'frmfluxo').then((response) => {
                if (response) {
                  let item = {};
                  ...
                  apiUpdate('Prevenda', item).then((response) => {
                    if (response.status === 200) {
                      if (response.data.status === 1) {
                        ...
                        apiInsert('PrevendaHistorico', item).then((response) => {
                          if (response.status === 200) {
                            const tipo = itemselec.tipoop;
                            const status = valuesfield[1];
                            apiFind('PrevendaStatus', 'TB01156_CODIGO,TB01156_PROPOSTA', '', "TB01156_CODIGO = '" + status + "' ").then(
                              (response) => {
                                if (response.status === 200) {
                                  ...
                                  if (proposta === 0) {
                                    ...
                                    apiFind('PrevendaTipo', 'TB01132_CODIGO,TB01132_MODELO', '', "TB01132_CODIGO = '" + tipo + "' ").then(
                                      (response) => {
                                        if (response.status === 200) {
            ...
            <Modal backdrop="static" size="xl" show={showemail} centered={true} onHide={handleCloseemail}>
              ...
              <ModalBody>
                <Email
                  ...
                  modulo="DataClient - Criação de Pré-Venda"
