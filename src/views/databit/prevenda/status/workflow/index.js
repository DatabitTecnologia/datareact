...
const PrevendaWorkflow = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList(
      'PrevendaStatus',
      'TB01156_CODIGO,TB01156_NOME',
      '',
      "TB01156_SITUACAO = 'A' AND TB01156_CODIGO <> '" +
        props.statusselec +
        "' " +
        "AND NOT EXISTS (SELECT TB01157_STATUS FROM TB01157 WHERE TB01157_STATUSFIM = TB01156_CODIGO AND TB01157_STATUS =  '" +
        props.statusselec +
        "') order by TB01156_NOME"
    ).then((response) => {
    ...
    apiList('PrevendaWordkflowVW', '*', '', "TB01157_STATUS= '" + props.statusselec + "' order by TB01156_NOME ").then((response) => {
      if (response.status === 200) {
        console.log(response.data);
  ...
  const Salvar = () => {
    ...
    apiExec("DELETE FROM TB01157 WHERE TB01157_STATUS = '" + props.statusselec + "' ", 'N').then((response) => {
      if (response.status === 200) {
        rowsselect.forEach((item) => {
          iteminsert['statusfim'] = item.codigo;
          iteminsert['status'] = props.statusselec;
          apiInsert('PrevendaWorkflow', iteminsert).then((response) => {
            if (response.status === 200) {
              setStatusprocessa('');
            }
          });
...
export default PrevendaWorkflow;
