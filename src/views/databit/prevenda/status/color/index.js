...
const PrevendaStatusColor = (props) => {
  ...
  useEffect(() => {
    ...
    apiFind(
      'PrevendaStatus',
      'TB01156_CODIGO,TB01156_NOME,TB01156_COLOR,TB01156_COLOR2',
      '',
      "TB01156_CODIGO = '" + props.statusselec + "' "
    ).then((response) => {
  ...
  const Salvar = (item) => {
    ...
    item.color2 = cor2;
    setCarregando(true);
    apiUpdate('PrevendaStatus', item).then((response) => {
      if (response.status === 200) {
        setCarregando(false);
        setItemselec(response.data);
        setShowcolor(false);
...
export default PrevendaStatusColor;
