...
const PrevendaNotificacao = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList(
      ...
      '*',
      '',
      "TB00035_SITUACAO = 'A' AND NOT EXISTS (SELECT TB01159_USER FROM TB01159 WHERE TB01159_USER = TB00035_NOME AND TB01159_STATUS =  '" +
        props.statusselec +
    ...
    apiList('PrevendaNotification', '*', 'TB01159_USER AS nome', "TB01159_STATUS= '" + props.statusselec + "' order by TB01159_USER ").then(
      (response) => {
        if (response.status === 200) {
          //console.log(response.data);
  ...
  const Salvar = () => {
    ...
    apiExec("DELETE FROM TB01159 WHERE TB01159_STATUS = '" + props.statusselec + "' ", 'N').then((response) => {
      console.log(response);
      if (response.status === 200) {
        rowsselect.forEach((item) => {
          iteminsert['user'] = item.nome;
          iteminsert['status'] = props.statusselec;
          apiInsert('PrevendaNotification', iteminsert).then((response) => {
            console.log(response);
            if (response.status === 200) {
              console.log(response.data);
              setStatusprocessa('');
...
export default PrevendaNotificacao;
