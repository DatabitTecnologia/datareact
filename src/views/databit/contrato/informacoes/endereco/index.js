...
const ContratoEndereco = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList(
      ...
      '',
      " TB02176_CONTRATO= '" + cabecalho[valuesname.indexOf('contrato')] + "' "
    ).then((response) => {
      ...
      setColumns([
        ...
        { headerClassName: 'header-list', field: 'nome', headerName: 'Descrição do Endereço', width: 374 },
  ...
  useEffect(() => {
    if (itemselec !== undefined) {
      ...
      valuesfield[12] = cabecalho[valuesname.indexOf('contrato')];
  ...
  const Incluir = () => {
    apiID('PrecontratoEndereco').then((response) => {
      if (response.status === 200) {
        ...
        setShowsave(true);
        try {
          document.getElementById('TB02176_NOME').focus();
        } catch (error) {
          //console.log(error);
        }
      }
    ...
    if (rows.length > 0 && valuesfield[0] !== '' && valuesfield[0] !== undefined) {
      Confirmation('frmendereco', 'Confirma a exclusão deste registro ?').then((result) => {
        if (result.isConfirmed) {
          setCarregando(true);
          apiDelete('ContratoSite', itemselec).then((response) => {
            if (response.status === 200) {
              setCarregando(false);
  ...
  const Salvar = () => {
    if (document.getElementById('TB02176_NOME').value === undefined || document.getElementById('TB02176_NOME').value === '') {
      setItemvariant(1);
      ...
      try {
        ...
        //console.log(item);
        if (inclusao) {
          apiInsert('ContratoSite', item).then((response) => {
            if (response.status === 200) {
              setCarregando(false);
          ...
          apiUpdate('ContratoSite', item).then((response) => {
            if (response.status === 200) {
              setCarregando(false);
              if (response.data.status === 1) {
                setDisabled(response.data.status === 1);
