...
const AcompanhamentoRequisicao = (props) => {
  ...
  useEffect(() => {
    setColumns([
      ...
      { headerClassName: 'header-list', field: 'VALOR', headerName: 'Valor', width: 135, type: 'number', decimal: 2 },
    ...
    setFields([
      ...
      },
      {
        ...
        widthfield: 40,
        measure: '40rem',
        itens: 'Suprimento,Grupo,Subgrupo,Marca,Operação,Equipamento',
        values: '1,2,3,4,5,6'
      }
    ]);
    setFields2([
  ...
  const Filtrar = () => {
    if (valuesfield !== undefined && valuesfield.length > 0) {
      setCarregando(true);
      let resultado = [];
      apiExec("exec SP02188 '" + contrato.codigo + "'," + valuesfield[1], 'S').then((response) => {
        ...
        if (response.status === 200) {
          ...
          if (resultado !== undefined && resultado.length > 0) {
            ...
            apiExec("exec SP02189 '" + contrato.codigo + "' ", 'S').then((response) => {
        ...
        <Row>
          <Col style={{ marginLeft: '5px', width: '50%' }}>
            <Card className="Recent-Users">
              <Card.Header>
                <Card.Title as="h5">Informações de Requisições</Card.Title>
              </Card.Header>
              <div>
...
export default AcompanhamentoRequisicao;
