...
const PreContratoNotificacao = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList(
      ...
      '*',
      '',
      "TB00035_SITUACAO = 'A' AND NOT EXISTS (SELECT TB01144_USER FROM TB01144 WHERE TB01144_USER = TB00035_NOME AND TB01144_STATUS =  '" +
        props.statusselec +
    ...
    apiList(
      'PrecontratoNotificacao',
      '*',
      'TB01144_USER AS nome',
      "TB01144_STATUS= '" + props.statusselec + "' order by TB01144_USER "
    ).then((response) => {
      if (response.status === 200) {
        //console.log(response.data);
  ...
  const Salvar = () => {
    ...
    apiExec("DELETE FROM TB01144 WHERE TB01144_STATUS = '" + props.statusselec + "' ", 'N').then((response) => {
      if (response.status === 200) {
        rowsselect.forEach((item) => {
          iteminsert['user'] = item.nome;
          iteminsert['status'] = props.statusselec;
          apiInsert('PrecontratoNotificacao', iteminsert).then((response) => {
            if (response.status === 200) {
              setStatusprocessa('');
            }
          });
...
export default PreContratoNotificacao;
