...
const PreContratoUser = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList('PrecontratoUser', '*', 'TB01139_USER AS nome', "TB01139_STATUS= '" + props.statusselec + "' order by TB01139_USER ").then(
      (response) => {
        if (response.status === 200) {
          //console.log(response.data);
  ...
  const Salvar = () => {
    ...
    apiExec("DELETE FROM TB01139 WHERE TB01139_STATUS = '" + props.statusselec + "' ", 'N').then((response) => {
      if (response.status === 200) {
        rowsselect.forEach((item) => {
          iteminsert['user'] = item.nome;
          iteminsert['status'] = props.statusselec;
          apiInsert('PrecontratoUser', iteminsert).then((response) => {
            if (response.status === 200) {
              setStatusprocessa('');
            }
          });
...
export default PreContratoUser;
