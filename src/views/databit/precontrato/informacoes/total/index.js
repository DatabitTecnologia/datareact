...
const PreContratoTotal = (props) => {
  ...
  const Filtrar = () => {
    ...
    apiList(
      ...
      '',
      "TB02265_CODIGO = '" + valuesfield[valuesname.indexOf('codigo')] + "' order by TB02265_NOMEITEM"
    ).then((response) => {
  ...
  const IncluirProduto = () => {
    ...
    setShowsave(true);
    try {
      document.getElementById('TB02265_TIPO').focus();
    } catch (error) {
      //console.log(error);
    }
  };
    ...
    if (rowsprod.length > 0 && valuesprod[0] !== '' && valuesprod[0] !== undefined) {
      Confirmation('frmbasic', 'Confirma a exclusão deste registro ?').then((result) => {
        if (result.isConfirmed) {
          ...
          apiDelete('PrecontratoItem', itemselecprod).then((response) => {
            if (response.status === 200) {
  ...
  const EditarProduto = () => {
    if (rowsprod.length > 0) {
      ...
      setValuesdisableprod([...valuesdisableprod]);
      try {
        document.getElementById('TB02265_QTAPROVADA').focus();
      } catch (error) {
        //console.log(error);
      }
    } else {
  ...
  const SalvarProduto = () => {
    setCarregando(true);
    if (
      document.getElementById('TB02265_TIPO').value === 0 &&
      (document.getElementById('TB02265_PRODUTO').value === undefined || document.getElementById('TB02265_PRODUTO').value === '')
    ) {
      setItemvariant(1);
      setMensagem('Campo PRODUTO é preenchimento obrigatório !');
      document.getElementById('TB02265_PRODUTO').focus();
    } else if (
      document.getElementById('TB02265_TIPO').value === 1 &&
      (document.getElementById('TB02265_CODSERV').value === undefined || document.getElementById('TB02265_CODSERV').value === '')
    ) {
      setItemvariant(1);
      setMensagem('Campo SERVIÇO é preenchimento obrigatório !');
      document.getElementById('TB02265_CODSERV').focus();
    } else {
      try {
        let item = {};
        ...
        if (inclusaoprod) {
          apiInsert('PrecontratoItem', item).then((response) => {
            if (response.status === 200) {
              if (response.data.status === 1) {
                setDisabledprod(response.data.status === 1);
                setItemselecprod(undefined);
          ...
          apiUpdate('PrecontratoItem', item).then((response) => {
            if (response.status === 200) {
              if (response.data.status === 1) {
                setDisabledprod(response.data.status === 1);
                setItemselecprod(undefined);
...
export default PreContratoTotal;
